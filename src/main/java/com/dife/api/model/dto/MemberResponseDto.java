...
public class MemberResponseDto {
	...
	public MemberResponseDto(Member member) {
		...
		if (member.getLanguages() != null && !member.getLanguages().isEmpty()) {
			this.languages =
					member.getLanguages().stream().map(Language::getName).collect(Collectors.toSet());
		}
		if (member.getHobbies() != null && !member.getHobbies().isEmpty()) {
			this.hobbies = member.getHobbies().stream().map(Hobby::getName).collect(Collectors.toSet());
		}
